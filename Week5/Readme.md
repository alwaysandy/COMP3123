...
Express router allows you to organize all your routes into different files instead of one monolithic app file, making it easier to read and navigate the codebase.
...
Since the error handling middleware will catch server errors, you can throw an error from the erroring code and a 500 (or whatever code is provided) error will be thrown by the middleware.

E.g reading from a file, if it doesn't exist an error will be automatically thrown and caught by the middleware.
---
...
On calling app.listen, it will first check to see if a port environment variable is set, and if not will default to port 8081. This is useful in case the server it's deployed to has a set port for express to send data on, instead of 8081 which might be used for a different program.
---
